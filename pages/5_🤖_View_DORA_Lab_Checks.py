   ...
   st.markdown(":red[Please sign in using your UNI_ID and UUID in the sidebar of the homepage.]")
         ...
         if mw_choice:
                if mw_choice == 'CMCW':
                     ...
                     st.markdown(":blue[If you are certain you ran tests in your ACME account but  they are NOT showing up here, revisit the :chains: and :link: pages to add your ACME account locator to your :link: row for CMCW.]")
		...
		# all_my_tests_df = session.table("AMAZING.APP.ALL_MY_TESTS").filter((col('uni_id')== st.session_state.uni_id) & (col('badge_acro')== mw_choice))
                ...
                if amt_rows > 0:    
                        ...
                        st.write("NOTE: If no rows are loaded make sure you have created your LINK row correctly. The LINK row for each workshop must be ACCURATE before DORA checks can be shown.")
