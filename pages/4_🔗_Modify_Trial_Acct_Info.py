...
# st.header(":red[NOTE: We are currently fixing an issue with this page. Please come back in 15 minutes.]")
...
def validate_acct_loc(acct_loc):
   if acct_loc == None:
      ...
      st.markdown(":x: :red[The ACCOUNT LOCATOR does not seem accurate. Please scroll down for guidance.]")
      st.session_state.aid_legit = False
   else: 
...
def validate_acct_id(acct_id):
   if acct_id == None:
      st.markdown(":x: :red[This field cannot be blank]")
   elif 'ACME' in acct_id:
      st.markdown(":x: :red[We need the ID of your MAIN or primary Snowflake Account in this field. Not the ID of your ACME account. Please scroll down for examples.]")
   elif len(acct_id) < 15 or len(acct_id) > 18:
      st.markdown(":x: :red[The ACCOUNT ID you entered does not seem accurate. Please scroll down for guidance and examples.]")
      st.session_state.al_legit = False
   elif acct_id.find(".") < 0:
      st.markdown(":x: :red[The ACCOUNT ID does not seem accurate. Please scroll down for guidance and examples.]")
      st.session_state.al_legit = False
   else: 
      st.markdown(":white_check_mark: :green[The ACCOUNT ID entered seems legit. You're awesome!]")
...
def validate_acme(acme_acct_loc):
   if acme_acct_loc is None:
      ...
      st.session_state.acme_legit = True   
   elif 'ACME' in acme_acct_loc:
      st.markdown(':x: :red[Your ACME ACCOUNT LOCATOR is not "ACME", that is the Account NAME. Please scroll down for guidance and examples.]')
   elif len(acme_acct_loc) < 7 or len(acme_acct_loc) > 8:
      st.markdown(":x: :red[Your ACME ACCOUNT LOCATOR does not seem accurate. Please scroll down for guidance and examples.]")
         ...
         if st.session_state.workshop_choice != '<Choose a Workshop>' and st.session_state.workshop_choice != ':red[NO WORKSHOP CHOSEN]':
            ...
            if st.session_state.al_legit == True and st.session_state.aid_legit==True:
               ...
               if st.session_state.workshop_choice == 'Badge 2: CMCW' and st.session_state.acme_legit == True and st.session_state.new_record == 'False':
                  update_sql = (f"call AMAZING.APP.CMCW_UPDATE_ACCT_INFO_SP(trim('{st.session_state.uni_id}'),trim('{st.session_state.uni_uuid}'),"
                                f" trim('{edited_acct_id}'),trim('{edited_acct_loc}'),trim('{edited_acme}'))")
                  st.write(update_sql)
                  ...
                  update_sql = (f"call AMAZING.APP.ADD_ACCT_INFO_SP(trim('{st.session_state.new_record}'),trim('{st.session_state.uni_id}'),'{st.session_state.uni_uuid}',"
                                f" trim('{st.session_state.workshop_choice}'), trim('{edited_acct_id}'), trim('{edited_acct_loc}'),'MAIN')")
                  # st.write(update_sql)
                  cur.execute(update_sql)
                  st.success('Snowflake Trial Account Workshop Data Updated', icon='🚀')
...
st.write("The ACCOUNT IDENTIFIER is a two-part code made up of the ORGANIZATION NAME and the ACCOUNT NAME. These can be customized to something meaningful for organizations by their account executives. They must be universally unique combinations. Clicking on the COPY symbol (as shown in the image below) will give you exactly what we need you to enter in the first box of the sub-form. Please use that copy button to get the full, two-part ACCOUNT ID. Then, paste it into the top field of the sub-form above. ") 
st.image('https://learn.snowflake.com/asset-v1:snowflake+X+X+type@asset+block@dil_1.png')
st.write("The ACCOUNT LOCATOR is a one-part code that cannot be changed. It is unique within a region (but may be duplicated across regions and clouds). For this reason, is it good to use in some instances but not others. We need this code in addtion to the ACCOUNT ID to track your lab work,  so please carefully enter the ACCOUNT LOCATOR in the second box of the sub-form above. ")
if 'workshop_choice' in st.session_state and  st.session_state.workshop_choice== "Badge 2: CMCW":
    st.markdown(":blue[If you are building your link row for the CMCW Workshop please note that we DO NOT need the ID for your ACME Account, we need your ACCOUNT LOCATOR. Your ACME account ID will have the same ORG ID as your primary account and the account name will be 'ACME'. No need to enter something we can easily infer. What we DO need you to enter in that third field of the sub-form is the ACCOUNT LOCATOR of the ACME account.]")

st.image('https://learn.snowflake.com/asset-v1:snowflake+X+X+type@asset+block@dil_3.png')
st.subheader('EXAMPLE ENTRY')
st.write(' This is what Tsai would enter based on the screenshots shown above:')
st.image('https://learn.snowflake.com/asset-v1:snowflake+X+X+type@asset+block@what_tsai_entered.png')
